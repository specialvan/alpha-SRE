# [PR] alpha-SRE 前端 V3.2 需求文档（Artifact-First / Mock-First）
...
- `OverviewSummary` 中与 review 时间窗口、最新 review、review 时间线相关的字段，只能在 provider 可从 artifact 元数据、filesystem mtime 或扩展 contract 稳定解析时间语义时提供；否则必须降级为总量、静态入口或不展示，不得伪造时间字段
...
Validation Center 的过滤维度必须按 provider 当前可稳定支持的能力分层：

- 必选：`failure class`
- 必选：`subject id`
- 必选：`affected field`
- 可选：`time range`（仅当数据源可解析或 provider 可稳定推导时启用）
- 可选：`replay operator id`（仅当 replay bundle/session 可解析时启用）
...
- 时间范围和 replay operator 过滤在数据不可推导时必须降级为不可用，而不是伪造字段
...
#### 目标
一眼看清系统当前健康度、最新 replay 结果、近期 gate 阻断、最新 incident，以及 review 样本入口。
...
- 最近活动流
  - artifact / replay / incident / release attempt 的时间线
  - review 仅在 provider 可稳定提供时间语义时进入时间线；否则退化为静态入口、样本总数或最近可见样本列表
- 快速入口
  - Artifact Browser
  - Snapshot Viewer
  - Replay Lab
...
- 当 review artifact 缺少稳定时间字段时，首页不得伪造“最近 24h review”KPI 或严格时间线排序；仅可展示总量、静态入口，或基于 filesystem mtime / 外部元数据的 best-effort 结果（若 provider 明确声明）
...
以下高级能力仅在数据源原生提供或 provider 可稳定推导时启用：
- 时间范围过滤
- 状态过滤

对于不同记录类型，前端应以“字段能力矩阵”方式展示支持能力，不得假定所有列表都天然具备同样的时间/状态字段。
...
- `mock provider` 与 `artifact provider` 均已实现，并能通过统一接口切换
- 基础单元测试覆盖率达到 60% 以上
...
| review artifact 缺少稳定时间元数据 | 首页 review KPI 与时间线无法严格排序 | provider 明确声明时间来源时做 best-effort；否则降级为总量或静态入口 |
...
这一版 V3.2 前端 PR 的目标不是做“漂亮的 dashboard”，而是把 alpha-SRE 后端已经存在的能力，变成一个能被 OnCall、运营、工程和复盘角色共同使用的叙事 SRE 控制台原型。
